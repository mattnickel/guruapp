        ...
        class MyChannelView < Grape::API
            ...
            resource :my_channel_view do
                ...
                def self.get_my_published_videos(userid)
                    ...
                    .where(["videos.user_id = :user_id and is_published = :is_published",{user_id: userid, is_published: true}])
                ...
                desc "Get my unpublished videos"
                get "my_unpublished_videos" do
                    current_user = User.find_by(authentication_token: headers['Token'])
                    unpublished_videos = MyChannelView.get_my_unpublished_videos(current_user.id)
              
                    render json: {
                    my_published_videos: unpublished_videos.video_list,
                    is_success: true,
                    status: :ok
                    }
                end

                def self.get_my_unpublished_videos(userid)
                    unpublished_videos = VideoList.new
                    my_unpublished_videos = Video.left_outer_joins(:viewings).select('videos.id AS id, videos.title, videos.author, COUNT(DISTINCT viewings.user_id) AS viewer_count')
                    .where(["videos.user_id = :user_id and is_published = :is_published",{user_id: userid, is_published: false}])
                    .group('videos.id, videos.title, videos.author')
                    .order('videos.created_at desc')

                    unpublished_videos.video_list = [];
                    my_unpublished_videos.each do |video_entry|
                        ...
                        my_video.thumbnail = video.image
                        my_video.record = video
                        unpublished_videos.video_list.push(my_video)  
                    end
                
                    return unpublished_videos
